f# Copyright 2008-2011 Nokia Networks
...
class TableElementKeywords(LibraryComponent):
    ...
    @keyword
    def get_table_cell(self, locator, row, column, index=None, loglevel='TRACE'):
    ...
    @keyword
    def get_table_cell_num(self, locator, colvalue="test-PC", loglevel='TRACE'):
        """Get the line number of the specified value
        ...
        """
        try:
            cells = self._get_all_rows(locator)
            self.info("get cells is {}".format(cells))
            for n, cell in enumerate(cells):
                self.info(cell.text)
                if colvalue in cell.text:
                    self.info("{0} in line{1} found".format(colvalue, n))
                    return n
        except AssertionError:
            self.log_source(loglevel)
            raise
    ...
    @keyword
    def get_tables_cell_num(self, locator, index, colvalue="test-PC", loglevel='TRACE'):
        """Get the line number of the specified value
        ...
        """
        try:
            cells = self._get_all_rows(locator, index)
            self.info("get cells is {}".format(cells))
            for n, cell in enumerate(cells):
                self.info(cell.text)
                if colvalue in cell.text:
    ...
    def _get_cell(self, locator, row, column, index=None):
        rows = self._get_rows(locator, row, index)
        print("rows is {}".format(rows))
        if len(rows) < abs(row):
    ...
    def _get_rows(self, locator, count, index=None):
        # Get rows in same order as browsers render them.
        if index is None:
            table = self.find_element(locator, tag='table')
        else:
            table = self.find_elements(locator, tag='table')[index]
    ...
    def _get_all_rows(self, locator, index=None):
        # Get rows in same order as browsers render them.
        if index is None:
            table = self.find_element(locator, tag='table')
        else:
        ...
        rows.extend(table.find_elements_by_xpath("./thead/tr"))
        rows.extend(table.find_elements_by_xpath("./tfoot/tr"))
        self.log(rows)
        return rows
    ...
    @keyword
    def tables_cell_should_contain(self, locator, row, column, expected, index=None, loglevel='TRACE'):
        """Verifies tables cell contains text ``expected``.

        Author:lipengb@uniontech.com
        See `Get Table Cell` that this keyword uses internally for
        an explanation about accepted arguments.
        """
        content = self.get_table_cell(locator, row, column, index, loglevel)
        if expected not in content:
            self.ctx.log_source(loglevel)
            raise AssertionError("Table '%s' cell on row %s and column %s "
                                 "should have contained text '%s' but it had "
    ...
    @keyword
    def tables_column_should_contain(self, locator, column, expected, index=-1, loglevel='TRACE'):
        """Tables should be contain text
        ...
        ``loglevel`` argument.
        """
        element = self._find_by_column(locator, column, expected, index)
        if element is None:
            self.ctx.log_source(loglevel)
            raise AssertionError("Table '%s' column %s did not contain text "
                                 "'%s'." % (locator, column, expected))
    ...
    @keyword
    def tables_column_should_not_contain(self, locator, column, expected, index=-1, loglevel='TRACE'):
        """Tables should not be contain text
        ...
        if element is not None:
            self.ctx.log_source(loglevel)
            raise AssertionError("Table '%s' column %s did contain text "
                                 "'%s'." % (locator, column, expected))

    @keyword
    def table_column_contain_isornot(self, locator, column, expected, index=None, loglevel='TRACE'):
        """
    ...
    @keyword
    def tables_should_contain(self, locator, expected, index=0, loglevel='TRACE'):
        """Verifies table contains text ``expected``.
        ...
        ``loglevel`` argument.
        """
        element = self._find_by_content(locator, expected, index)
        if element is None:
            self.ctx.log_source(loglevel)
            raise AssertionError("Table '%s' did not contain text '%s'."
                                 % (locator, expected))

    def _find_by_content(self, table_locator, content, index):
        return self._find(table_locator, 'xpath:.//*', content, index)
    ...
    def _find_by_column(self, table_locator, col, content, index=None):
        position = self._index_to_position(col)
        locator = '//tr//*[self::td or self::th][{}]'.format(position)
        self.log(locator)
        return self._find(table_locator, locator, content, index)
    ...
    def _find(self, table_locator, locator, content, index=None):
        if index == None:
            table = self.find_element(table_locator)
        else:
            tables = self.find_elements(table_locator)
            self.log("table num is {}".format(len(tables)))
            table = tables[index]
        self.log("locator is {}".format(locator))
        elements = self.find_elements(locator, parent=table)
        self.log(elements)
        for element in elements:
            self.log("text:{}".format(element.text))
            if content is None:
                return element
