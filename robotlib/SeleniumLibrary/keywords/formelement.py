...
class FormElementKeywords(LibraryComponent):
    ...
    @keyword
    def input_texts(self, locator, text, index, clear=True):
        """Text input when handling repeated elements
        ...
        """
        self.info("Typing text '%s' into text field '%s'." % (text, locator))
        self._input_text_into_text_field(locator, text, clear, index)
    ...
    def _input_text_into_text_field(self, locator, text, clear=True, elemindex=-1, disable_log=False):
        if int(elemindex) < 0:
            element = self.find_element(locator)
        else:
            element = self.find_elements(locator)[elemindex]
        if is_truthy(clear):
            element.clear()
        if disable_log:
